# -*- coding: utf-8 -*-
"""
Created on Fri Nov 12 00:23:38 2021
...
use_words = True   # Use words instead of characters (slower speed, bigger vocabulary)
data_file = "txtfile.txt"  # Source text file
starlist = ("новая","группа","соло","проект", "а", "певец", "певица", "премия", "шоу")
seed = starlist[random.randint(0,len(starlist)-1)]  # Initial symbol of the text
...
# Generate phrases
#res = generate(predict_model, vectorizer, seed=starlist[random.randint(0,len(starlist)-1)], length=random.randrange(80))
#print(seed, ' ',res.split('.')[0], '. ',res.split('.')[1], '. ',res.split('.')[2],sep='')
